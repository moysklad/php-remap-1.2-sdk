    ...
    public static function deserialize($data, $class, $httpHeaders = null)
    {
        ...
        if (method_exists($class, 'getAllowableEnumValues')) {
            ...
            if (!empty($discriminator) && isset($data->{$discriminator}) && is_string($data->{$discriminator})) {
                $mappings = $class::openAPIMappings();
                $discriminatorType = null;
                if (array_key_exists($data->{$discriminator}, $mappings)) {
                    $discriminatorType = $mappings[$data->{$discriminator}];
                   // В OpenApi нельзя задать дефолтный маппер когда придет неизвестный тип для маппинга.
                   // Для сохранения ОС для старых версий сдк нужно добавить маппер на новые неизвестные типы
                } elseif (array_key_exists('undefined', $mappings)) {
                    $discriminatorType = $mappings['undefined'];
                }
                if ($discriminatorType != null) {
                    $subclass = '\OpenAPI\Client\Model\\' . $discriminatorType;
